...
function matrixHasHoles(matrix) {
    let majorDim = maxMajorDimension(matrix);
    let minorDim = maxMinorDimension(matrix);

    for (let x = 0; x < majorDim; x++) {
        let row = matrix[x];
        if (row === undefined) {
            return true;
        }

        for (let y = 0; y < minorDim; y++) {
            let cell = row[y];
            if (cell === undefined) {
                return true;
            }
...
function scrapeScriptVariable(variable) {
        ...
        if (script.innerHTML.indexOf(variable) !== -1) {
            ...
            for (let line of lines) {
                if (line.indexOf(assignment) !== -1) {
                    ...
                    let expression = rhs.substr(0, rhs.length - 1);

                    // Remove a right hand size fallback `||` operator
                    var literal;
                    let operatorIndex = expression.indexOf("||");
                    if (operatorIndex !== -1) {
                        literal = expression.substr(0, operatorIndex).trim();
                    } else {
                        literal = expression;
                    }
    ...
    console.log("Failed to find scrapable variable:", variable);
    return null;
}
...
function scrapeWeatherCode() {
    return scrapeScriptVariable("weatherCode");
}
...
async function fetchProfileSettings() {
    ...
    const response = await fetch("https://awbw.amarriner.com/api/user/userinfo.php",
        {
            method: "POST",
    ...
    return response.json().then((settings) => {
        return settings;
    });
}
