const kCountries = [
    ...
    {turnOrder: 18, code: "ne", flatName: "noireclipse",     name: "Noir Eclipse"},
    {turnOrder: 19, code: "sc", flatName: "silverclaw",      name: "Silver Claw"},
    {turnOrder: 20, code: "uw", flatName: "umberwilds",      name: "Umber Wilds"},
];

// https://stackoverflow.com/questions/66978794/how-to-parse-a-non-json-string-to-a-object-in-javascript
// Since we cannot use eval(), we get the script tag that contains the armies object and parse it
document.querySelectorAll("script").forEach((script) => {
    if (script.src == "") {
        if (script.innerHTML.includes("armies")) {
            // Matches "const armies = { ... };"
            const armiesJSON = script.innerHTML.replaceAll("\n", "").match(/(?:const armies) = ({.*});/)[1];

            // Add quotes to the keys to make it valid JSON
            const keyFinderRegEX = /([{,]\s*)(\S+)\s*(:)/mg;
            const convertedJSON = armiesJSON.replace(keyFinderRegEX, '$1"$2"$3');
            const parsedArmies = JSON.parse(convertedJSON);

            // Add the countries to the kCountries array
            for (const flatName of Object.keys(parsedArmies)) {
                const alreadyExists = kCountries.some((kCountry) => kCountry.flatName == flatName)
                if (alreadyExists) continue;

                const armyData = parsedArmies[flatName];
                const countryData = {
                    turnOrder: armyData.order,
                    code: armyData.short,
                    flatName: flatName,
                    name: flatName.charAt(0).toUpperCase() + flatName.slice(1)
                }
                kCountries.push(countryData);
                console.log("Added country: ", countryData);
            }
        }
    }
});
...
const kTerrainIds = {
    ...
    "base":       [35, 39, 44, 49, 54, 82, 87, 92, 97, 118, 123, 150, 157, 164, 171, 182, 189, 197, 204, 211],
    "port":       [37, 41, 46, 51, 56, 84, 89, 94, 99, 121, 126, 155, 162, 169, 176, 187, 194, 202, 209, 216],
    "silo":       [111, 112],
    ...
    "properties": [34, 36, 38, 40, 42, 43, 45, 47, 48, 50, 52, 53, 55, 57, 81, 83, 85, 86, 88,
                   ...
                   190, 191, 192, 193, 196, 198, 199, 200, 201, 203, 205, 206, 207, 208, 210, 212, 213, 214, 215],
};
